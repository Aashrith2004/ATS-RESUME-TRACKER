...
The first line indicates the percentage match with the job description (JD). The second line presents a list of missing keywords.
...
with col1:
    ...
    if submit:
        if uploaded_file is not None:
            ...
            response = get_gemini_response(input_prompt.format(jd=jd, text=text))
            st.subheader(response)
        else:
            st.error("Please upload a PDF resume.")

with col2:
    st.image('https://www.pharmacymentor.com/wp-content/uploads/2021/11/Bespoke-Pharmacy-Marketing-1030x1030.png', use_column_width=True)
